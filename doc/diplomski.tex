...
Razvoj konvolucijskih mreža počeo je osamdesetih godina prošlog stoljeća pojavom \textit{neocognitron}[citat?]-a--neuronske mreže inspirirane biološkim stanicama vidne kore mozga. Krajem devedesetih godina se pojavljuje konvolucijska neuronska mreža LeNet5. LeNet5 mreža je vrlo uspješno raspoznavala rukom pisane znamenke te je ova mreža bila početna točka za daljnja istraživanja drugih neuronskih mreža. \par
...
\begin{figure}[H]
  ...
  \caption{Primjeri lažno pozitivnog i lažno negativnog napada. Napad proveden na \textit{ResNet50V2} mreži korištenjem \textit{Carlini and Wagner} $\ell_{2}$ napada opisanog u \ref{cw}.}
...
\begin{figure}[H]
  ...
  \hfill
  \subfloat[Diferencijabilni binarni klasifikator. U svakom koraku se za trenutnu točku aproksimira funkcija klasifikatora s hiperravninom i napravi projekcija na pravac koji siječe $\mathbb{R}^{n}$ (narančaste boje).]{\includegraphics[width=0.49\textwidth]{img/other/deepfool_differential_binary.png}\label{fig:deepfool_2d}}
...
Bitno je naglasiti da ni ovako jaki napad ne dolazi besplatno. U usporedbi s napadima bijele kutije, broj evaluacija mreže je veći za nekoliko redova veličine (ukoliko je isti cilj, na primjer isti iznos norme). Za DeepFool napad, autorima je u prosjeku trebalo $7$ prolaza kroz mrežu (predikcija) i $37$ prolaza unazad (za računanje gradijenata), dok je za granični napad slične norme potrebno $1200000$ prolaza kroz mrežu i $0$ prolaza unazad (jer je napad na model crne kutije). Ovo je u slučaju da se žele iznimno dobri rezultati. Na slici \ref{fig:boundary_triple} je napad i nakon $8000$ iteracija postigao vrlo dobar rezultat, a još su k tome i slike birane tako da problem bude teži -- ciljana slika ima vrlo jednostavnu jednobojnu pozadinu i napad je svejedno iznimno uspješan. Jedino se u slučaju ljame vidi obris, no i to bi zasigurno nestalo nakon mnogo iteracija. Za $8000$ iteracija napada potrebno vrijeme izvršavanja bilo je preko $8$ sati na ove tri slike.
