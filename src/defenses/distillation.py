...
def eval():
    ...
    model_final = tf.keras.models.load_model('distilation/model_final50')
    model_clean = tf.keras.models.load_model('distilation/model_clean')
    ...
    art_model1 = TensorFlowV2Classifier(model_final, nb_classes=10, input_shape=(32, 32, 3),
                                        clip_values=(0, 255), loss_object=loss_object)
    a, b = fgm(art_model1, x_test, eps=2)
    y_pred = np.argmax(art_model1.predict(a), axis=1)
    acc = np.sum(y_pred == y_test) / len(y_test)
    print('fgm eps2 on distilled %.4f' % acc)

    a, b = fgm(art_model1, x_test, eps=5)
    y_pred = np.argmax(art_model1.predict(a), axis=1)
    acc = np.sum(y_pred == y_test) / len(y_test)
    print('fgm eps5 on distilled %.4f' % acc)

    a, b = fgm(art_model1, x_test, eps=10)
    y_pred = np.argmax(art_model1.predict(a), axis=1)
    acc = np.sum(y_pred == y_test) / len(y_test)
...
def eval_carlini():
    ...
    model_final = tf.keras.models.load_model('distilation/model_final100')
    model_clean = tf.keras.models.load_model('distilation/model_clean')
    ...
    art_model1 = TensorFlowV2Classifier(model_final, nb_classes=10, input_shape=(32, 32, 3),
                                        clip_values=(0, 255), loss_object=loss_object)
    a, b = cw_l2(art_model1, x_test[:8], max_iter=1000)
    y_pred = np.argmax(art_model1.predict(a), axis=1)
    acc = np.sum(y_pred == y_test[:8]) / len(y_test[:8])
    print(acc)
...
def train():
    ...
    # prob_model.save('distilation/model_clean')

    lambda_layer = tf.keras.layers.Lambda(lambda x: x / 50)
    ...
    model_final.save('distilation/model_final50')
