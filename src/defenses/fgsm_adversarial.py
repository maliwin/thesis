...
def test_fgsm(model):
    ...
    attacks = FastGradientMethod(art_model, eps=0.05)
    adv = attacks.generate(x_test)
    y_pred = np.argmax(model.predict(adv), axis=1)
    acc = np.sum(y_pred == y_test) / len(y_test)
    print('acc %.4f' % acc)

    attacks = FastGradientMethod(art_model, eps=0.1)
    adv = attacks.generate(x_test)
    y_pred = np.argmax(model.predict(adv), axis=1)
    acc = np.sum(y_pred == y_test) / len(y_test)
    print('acc %.4f' % acc)

    attacks = FastGradientMethod(art_model, eps=0.2)
    adv = attacks.generate(x_test)
    y_pred = np.argmax(model.predict(adv), axis=1)
    acc = np.sum(y_pred == y_test) / len(y_test)
    print('acc %.4f' % acc)
...
if __name__ == '__main__':
    setup_logging()
    # adv_train(epochs=25, eps=0.2, name='epochs25_eps_2')
    # adv_train(epochs=50, eps=0.2, name='epochs50_eps_2')
    # adv_train(epochs=25, eps=0.3, name='epochs25_eps_3')
    # adv_train(epochs=50, eps=0.3, name='epochs50_eps_3')
    model = tf.keras.models.load_model('fgsm_training/' + 'epochs25_eps_2')
    test_fgsm(model)
    model = tf.keras.models.load_model('fgsm_training/' + 'epochs50_eps_2')
    test_fgsm(model)
    model = tf.keras.models.load_model('fgsm_training/' + 'epochs25_eps_3')
    test_fgsm(model)
    model = tf.keras.models.load_model('fgsm_training/' + 'epochs50_eps_3')
    test_fgsm(model)
